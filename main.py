...
byte_track = sv.ByteTrack(frame_rate=video_info.fps, track_thresh=0.30,
                          match_thresh=0.8, track_buffer=video_info.fps)
line_zone = sv.LineZone(start=LINE_START, end=LINE_END)
bounding_box_annotator = sv.BoundingBoxAnnotator(
    thickness=1, color_lookup=sv.ColorLookup.TRACK)
...
for _ in tqdm(range(video_info.total_frames), desc="Rendering videos with Bounding Box: "):
    ...
    results = model(frame, classes=[2, 5, 7], verbose=False, device='mps')[0]
    ...
    if frame_num % video_info.fps == 0:
        in_count.append(line_zone.in_count)
        out_count.append(line_zone.out_count)
...
total_count_data = {
    'time': range(len(in_count)),
    'in_count': in_count,
    'out_count': out_count
}

df = pd.DataFrame(total_count_data)
df.to_csv(os.path.join(CURRENT_DIRECTORY,
          'datasets/data/count.csv'), index=False)
